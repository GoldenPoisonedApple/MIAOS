...
class MIA_LIRA(MIA_Attack):
    ...
    def attack(
        ...
        # → つまりそのインデックスに対応する正解クラスインデックスが[]内で作成
        # → 確立達の中から、正解ラベルに対応する正解クラス確率が抽出される
        target_in_prob = target_in_preds[
            torch.arange(len(target_in_labels)), target_in_labels
        ]
        target_out_prob = target_out_preds[
            torch.arange(len(target_out_labels)), target_out_labels
        ]
        # ロジット変換
        ...
        lira_trues = np.concatenate(
            [np.ones(len(in_z_scores)), np.zeros(len(out_z_scores))]
        )
