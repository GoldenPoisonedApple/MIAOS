...
外部マスク PNG で形状を定義し、Normalize 前の PIL 段階で透かしを合成する。設定は `CreateExperimentRequest.hyperparameters.watermark` から読み取る（OpenAPI スキーマ変更は不要）。
...
```json
{
  "watermark": {
    "enabled": true,
    "mask_path": "assets/masks/circle_full.png",
    "color": [255, 255, 255],
    "opacity": 0.6,
    "position": [0, 0],
    "apply_to": ["target_train", "shadow_train"],
    "fraction": 1.0,
    "seed_offset": 0
  }
}
...
|------|------|
| `enabled` | 透かし ON/OFF |
| `mask_path` | マスク PNG（白=透かし領域、alpha チャンネル優先） |
| `color` | RGB 0–255 |
| `opacity` | ブレンド強度 0.0–1.0 |
| `position` | マスク左上座標 `[x, y]`（32×32 キャンバス内） |
| `apply_to` | `target_train`, `target_test`, `shadow_train`, `shadow_test` |
| `fraction` | 各対象分割内の透かし付与割合 |
| `seed_offset` | 透かし対象選定シード = `seed + seed_offset` |

**マスク解決順序**: ベース実験キャッシュ → プロジェクトルート相対 → 絶対パス。
...
**処理フロー**: PIL 読み込み →（対象インデックスなら）`ShapeWatermark` 合成 → `ToTensor` → `Normalize`。学習・評価ローダーで同一の透かし判定を適用する。
