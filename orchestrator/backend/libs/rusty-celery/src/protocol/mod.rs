        ...
        let msg_json_value = json!({
            ...
            "properties": json!({
                ...
                // "delivery_info": self.properties.delivery_info.clone().or(delivery_info).and_then(|i| serde_json::to_value(i).ok()).unwrap_or(Value::Null)
								// Modified by Ito 2026/05/13
								// delivery_infoがない場合、Nullという仕様だったが、PythonのCeleryではexchangeキーがないとエラーとなる
								// そのため、ない場合ははデフォルト値を返すように修正
								"delivery_info": self.properties.delivery_info.clone().or(delivery_info).and_then(|i| serde_json::to_value(i).ok()).unwrap_or_else(|| json!({
										"exchange": "",
										"routing_key": "celery"
									})
								)
