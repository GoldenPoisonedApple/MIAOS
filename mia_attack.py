...
class MIA_Attack(ABC):
	...
	def train_target_model(self, target_model: nn.Module):
		...
		self.logger.info(f"Target Result -> Train: {train_acc:.4f}, Test: {test_acc:.4f} (Gap: {train_acc - test_acc:.4f})")
	...
	def comprehensive_evaluate(self, scores: np.ndarray, trues: np.ndarray):
		...
		plt.axvline(x=0.0001, linestyle='--', linewidth=1, color='blue', label='0.01% FPR')
		...
		tpr_at_01_fpr, threshold_at_01_fpr = MIA_Attack.get_tpr_and_threshold(fpr, tpr, thresholds, 0.001)
		tpr_at_001_fpr, threshold_at_001_fpr = MIA_Attack.get_tpr_and_threshold(fpr, tpr, thresholds, 0.0001)

		self.logger.info(f"TPR: {(tpr_at_1_fpr * 100):.4f}% at 1.0% FPR, Threshold: {threshold_at_1_fpr:.4f}")
	...
	def get_tpr_and_threshold(fpr, tpr, thresholds, target_fpr):
		...
		idx = tpr[mask].argmax()          # 条件内で最大TPRの位置 [0.8, 0.9, 0.7, 0.6] → 0.9の位置=idx = 1
		selected_indices = np.where(mask)[0]	# 条件内でTrueのインデックスを取得 [True, True, False, True] → [0, 1, 3]
		best_idx = selected_indices[idx]  # 元のインデックスに戻す [0, 1, 3] → 1番目の = 1
